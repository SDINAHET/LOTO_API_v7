...
function ensureDeletedToggleUI() {
      ...
      if (!isReadOnly) {
        ...
        btnDel.addEventListener("click", async () => {
          ...
          try {
            ...
            // ✅ refresh propre depuis le backend
            await loadDbData();
          } catch (e) {
            console.error(e);
  ...
  async function saveDbModal() {
    ...
    try {
      ...
      // ✅ refresh tickets modal en arrière-plan si on vient du modal Tickets
      try {
        const ctx = window.__ticketsModalCtx;
        if (dbCurrentResource === "tickets" && ctx?.email) {
          refreshTicketsModalForEmail(ctx.email).catch(() => {});
        }
      } catch {}

      // ✅ rester dans le modal si flag activé
      if (__stayInEditAfterSave) {
        // on reste dans "Modifier"
        return;
      }

      // sinon : fermeture normale
      closeDbModal();
        ...
        btnDel.addEventListener("click", async () => {
          ...
          btnDel.addEventListener("click", async () => {
            ...
            const ok = await confirmDialog({
              ...
              message: `Supprimer ce ticket (#${t.id}) ?`,
              okText: "Supprimer",
              cancelText: "Annuler",
              danger: true,
            });
            ...
            try {
              ...
              // ✅ refresh du modal tickets sans fermer
              if (userEmail) {
                await refreshTicketsModalForEmail(userEmail);
              }

              // ✅ si on est sur la ressource tickets dans le tableau, refresh aussi
              if (dbCurrentResource === "tickets") {
                await loadDbData();
              }
            } catch (e) {
